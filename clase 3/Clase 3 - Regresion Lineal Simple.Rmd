...
```{r, message=FALSE}
encuesta <- read_csv("../Fuentes/regresion_simple/encuesta_sueldos_sysarmy_2s2020.csv")
encuesta %>%
  glimpse()
```
...
Respecto de nuestra variable a predecir, el **salario bruto**, observamos:
  ...
  * Todas las variables (excepto el salario neto) presentan una correlación muy baja con esta variable.
...
Existen más respuestas de *Data Scientist/Data Engineer* que de *BI Analyst/Data Analyst* aunque es una diferencia muy pequeña.
  
Parece existir:
  
  * Un outlier severo en la variable **años en el puesto actual**.
...
**Caso particular: Años de experiencia en el puesto actual**
...
```{r}
ggplot(encuesta_ds_relevantes, aes(y=anos_en_el_puesto_actual)) + geom_boxplot() + theme_bw()
```

```{r}
encuesta_ds_relevantes %>% filter(anos_en_el_puesto_actual>40)
```

En este caso el registro presenta multiples inconsistencias por lo tanto decidimos eliminarlo.
...
encuesta_ds_filtrada = encuesta_ds_relevantes %>%
                          filter(sueldo_dolarizado==0, #Eliminamos los sueldos dolarizados
                                 ...
                                 anos_en_el_puesto_actual<40) %>%  # Error de carga
...
Respecto de nuestra variable a predecir, el **salario bruto**, observamos:
  ...
  * Todas las variables (excepto el salario neto) siguen presentando una correlación general baja con esta variable.
  
  * Sin embargo, los años de experiencia en la empresa actual y los años de experiencia en el puesto actual presentan un correlación positiva moderada para el grupo de *BI Analyst/Data Analyst*.
  
  * Siguen existiendo outliers severos.
...
Existen varios pares de variables que presentan cambios entre la correlación de Pearson y la de Spearman, por ejemplo la correlación entre el salario bruto y la edad.
...
Son `r outliers_sueldo %>% nrow()` registros en total. Existen registros que parecen ser errores de carga y otros que parecen ser outliers "genuinos".
...
```
Vemos que hasta el percentil 1 los valores son anormalmente bajos pero no podían ser filtrados por el criterio anterior. Un valor apropiado para filtrar valores bajos parece ser el del percentil 2. Por lo que definimos este valor como límite inferior. 

```{r}
# Seleccionamos el valor del salario bruto del percentil 2
limite_inferior_percentil = percentiles_salario_bruto %>% 
                                filter(salario_bruto_vec_q==0.02) %>%
...
Ahora, respecto de nuestra variable a predecir, el **salario bruto**, observamos:
  ...
  * Existe una correlación positiva baja con los años en la empresa y los años en el puesto. Respecto a esta última variable existe una  diferencia en la correlación entre los dos perfiles. 
...
matriz.correl.pe
```
Como ya vimos en la sección inmediatamente anterior, nuestra variable de interés (el **salario_bruto**) tiene una correlación positiva (método de Pearson) MODERADA con las variables de **edad**: 0.39 y **años de experiencia**: 0.46 y BAJA con los **años en la empresa actual**: 0.20 y los **años en el puesto actual**: 0.23.
...
El modelo nos muestra la llamada utilizada y los valores de los coeficientes estimados: $\hat{\beta_0} =13506$ y $\hat{\beta_1} = 2827$.
...
```
  ...
  * Teóricamente el valor $\hat{\beta_0} = 13505.51$ es el salario bruto **esperado** para una persona recién nacida (0 años). Por este motivo este valor carece de interpretación práctica.
  
  * El valor de $\hat{\beta_1}$ indica que por cada aumento de un año de edad el salario neto **esperado** aumenta en 2826.66 pesos  
...
```
  ...
  * El valor de $\hat{\beta_0}$ indica que el salario bruto **esperado** es de $ 79503.83 para una persona con perfil de Data Scientist/Engineer o Data Analyst sin experiencia laboral.
  
  * El valor de $\hat{\beta_1}$ indica que por cada aumento de un año de experiencia el salario bruto **esperado** aumenta en $3370.48.  
...
```
  ...
  * El valor de $\hat{\beta_0}$ indica que el salario neto **esperado** es de $97064.91 para una persona con perfil de Data Scientist/Engineer o Data Analyst que recién comienza en una empresa. 
  
  * El valor de $\hat{\beta_1}$ indica que por cada aumento de un año de experiencia en la misma empresa el salario neto **esperado** aumenta en $1896.04.  
