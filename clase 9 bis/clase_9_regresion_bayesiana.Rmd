...
Trabajaremos con los datasets de salarios de la temporada 2022-2023 de la NBA y las estadísticas por partido de la temporada 2021-2022. La construcción estos datasets se encuentran en 
...
tidy(modelo_simple_puntos, conf.int = TRUE)
```
...
regresion_bayesiana_default <- brm(
  formula = salario ~ PTS,
...
```{r}
summary(regresion_bayesiana_default)
```
# Tidybayes
...
```
```{r}
get_variables(regresion_bayesiana_default)
```


```{r}
regresion_bayesiana_default %>%
...
```{r}
plot(conditional_effects(regresion_bayesiana_default), ask = FALSE)
```
...
¿Qué pasa con nuestro modelo si realizamos un warmup pequeño?
...
```{r, echo=TRUE, results='hide'}
regresion_bayesiana_unica_cadena <- brm(
  formula = salario ~ PTS,
  data = nba_salarios_estadisticas_partido,
  chains = 1,
  cores = 6,
...
```{r}
summary(regresion_bayesiana_unica_cadena)
```
```{r}
plot(regresion_bayesiana_unica_cadena, ask = FALSE)
```

## Pocas iteraciones

¿Qué pasa con nuestro modelo si realizamos un pocas iteraciones pequeño?

```{r, echo=TRUE, results='hide'}
regresion_bayesiana_pocas_iteraciones <- brm(
  formula = salario ~ PTS,
  data = nba_salarios_estadisticas_partido,
  iter = 400,
  cores = 6,
...
```
```{r}
summary(regresion_bayesiana_pocas_iteraciones)
```
