...
![Fuente: www.chaseyoursport.com](posiciones_basket.jpg)
...
```{r}
top_players = nba %>% arrange(desc(salario)) %>% select(jugador) %>% slice(1:10) %>% pull()
...
ggplot(nba, aes(Pos, salario, fill=Pos)) +
  geom_boxplot() +
  geom_text(aes(label=ifelse((salario>30500000) & jugador %in% top_players,as.character(jugador),'')),hjust=1.1,vjust=0, size=3) +
  theme_bw() +
...
* Las mismas cuatro variables tienen coeficientes significativos: **BLK**, **G** y **Age**.
...
Han quedado 19 variables y el modelo explica el 74% de la deviance.
...
```
En este caso conserva todas las variables, obteniendo un porcentaje de deviance explicado muy similar a Lasso: 73,6%.
