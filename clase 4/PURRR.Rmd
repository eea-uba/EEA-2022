...
```{r}
# cargamos el dataset
encuesta_sueldos_limpia = read_csv("../Fuentes/regresion_simple/encuesta_ds_limpia.csv")
encuesta_sueldos_limpia <- encuesta_sueldos_limpia %>%
  # creamos variable perfil BI/DA vs DS/DI
  mutate(perfil = case_when(perfil == "BI Analyst / Data Analyst" ~ "BI/DA", TRUE ~ "DS/DE"))
```
...
Podemos usar la función map para calcular distintos modelos. Por ejemplo, calculemos un modelo distinto según el perfil (DA y DS). En este caso pasamos como función un modelo lineal simple:
...
Podemos trabajar sobre esta lista de modelos de distintas maneras. Por ejemplo, podemos acceder a la información de los coeficientes estimados de ambos modelos usando también la función `map`
...
También podemos aplicar la función `summary` y acceder a la información de $R^2$ de ambos modelos para evaluar su capacidad explicativa (en este caso como son modelos simples podemos utilizar esta métrica).
...
Mientras el modelo de edad para los perfiles BI/DA logra explicar el `r round((modelos_por_perfil %>% map(summary) %>% map_dbl("r.squared"))[1]*100,2)`% de la variabilidad del fenómeno, el modelo de edad para los DS/DE logra captar un porcentaje menor de la variabilidad (`r round((modelos_por_perfil %>% map(summary) %>% map_dbl("r.squared"))[2]*100,2)`%). 
...
```{r}
modelos_por_perfil %>% 
  map_dfr(broom::glance, .id="modelo")
```
