...
En este modelo, el p-valor asociado al test de significatividad individual de $\beta_1$ es el mayor que hemos obtenido pero sigue siendo posible rechazar la hipótesis nula.
...
Observamos que el modelo que utiliza los años de experiencia como variable explicativa tiene el mayor R-cuadrado: $R^2=0.1373$ y por lo tanto es el mejor modelo entre los que probamos para **explicar** el salario bruto del grupo de personas que estamos analizando.
...
1) **Residuos** vs **valores predichos**:  Parece existir cierta estructura en los datos dado que hay una curvatura  

2) **Normal QQ plot**: los extremos no se ajustan a la distribución teórica

4) **Residual** vs **leverage**: Considerando el criterio que una observación es potencialmente influyente cuando $h_{ii}>0.5$ concluimos que no existen puntos que puedan ser consideradas influyentes. Sin embargo, observando la distribución de las observaciones se destaca la observación 97.

```{r}
#Miremos la observacion 97 en detalle
encuesta_sueldos[97,]
```

**Diagnóstico**: el modelo no cumple con los supuestos del modelo lineal. Parecen existir tres problemas situaciones: violación del supuesto de linealidad de la esperanza condicional, falta de normalidad y presencia de observaciones de alto leverage
...
Para mayor detalle de cómo sería la distribución de los residuos según el tipo de QQ-plot, sugerimos ver los ejemplos en [Quantile-Quantile Plots](https://www.ucd.ie/ecomodel/Resources/QQplots_WebVersion.html#right-skewed-data).
...
Dado que estamos trabajando con un modelo de regresión podemos observar este punto y la recta del modelo. 

```{r}
ggplot(data=encuesta_aumentada, aes(x=anos_de_experiencia, y=salario_bruto)) +
  # Graficamos los puntos
  geom_point() +
  # Graficamos el modelo
  geom_abline(intercept = modelo_experiencia$coefficients[1], slope = modelo_experiencia$coefficients[2], color='forestgreen', size=1) +
  # Graficamos el promedio de la variable x
  geom_vline(xintercept = mean(encuesta_aumentada$anos_de_experiencia), color='firebrick', linetype='dashed', size=1) +
  # Etiquetamos el punto relevante
  geom_text(aes(label=ifelse(.hat>0.10 ,as.character(id),'')),hjust=0,vjust=1.4, size=3) +
  theme_bw() +
  labs(title = "Leverage: Modelo Años de Experiencia", x="Años de Experiencia", y="Salario Bruto")
```

Versiones anteriores de la notebook se pueden encontrar en:

[EEA-2021](https://eea-uba.github.io/EEA-2021/clase%204/Clase-4---Regresion-Lineal-Simple-Eval.html): Elaborada por Juan Manuel Barriola y Sofía Perini

[EEA-2020](https://eea-uba.github.io/EEA-2020/clase%204/Clase-4---Regresion-Lineal-Simple-Eval.html): Elaborada por Juan Manuel Barriola y Sofía Perini 
